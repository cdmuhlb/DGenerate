...
class BogackiShampineStepper(ode: Ode, element: ActorRef, controller: ActorRef) extends Actor {
  ...
  def idle: Receive = {
    case TakeStep(dt, state1, k1) =>
      ...
      context.become(stage2(dt, state1, k1))
  }

  def stage2(dt: Double, state1: OdeState, k1: FieldVec): Receive = {
    case Stage2Result(k2) =>
      ...
      context.become(stage3(dt, state1, k1, k2))
  }

  def stage3(dt: Double, state1: OdeState, k1: FieldVec,
      k2: FieldVec): Receive = {
  ...
  def stage4(dt: Double, state1: OdeState, k1: FieldVec,
    ...
    case Stage4Result(state4, k4) =>
      ...
      controller ! ErrorEstimate(err)
      element ! AdvanceState(state4, k4)
      context.become(idle)
  }
